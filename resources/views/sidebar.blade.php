                ...
                <li>
                    ...
                    @if(Auth::check() && Auth::user()->role === 'pm')
                        <a href="{{ route('events.create') }}"
                            class="block ml-6 px-2 py-1 text-green-600 hover:underline text-sm">
                            + Buat Event Baru
                        </a>
                    @endif
                </li>
    ...
    <div class="p-4 border-t">
        ...
        @if(Auth::check())
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit"
                    class="w-full text-center px-4 py-2 rounded bg-red-50 text-red-600 hover:bg-red-100 transition mt-2">
                    Logout
                </button>
            </form>
        @else
            <a href="{{ route('login') }}"
                class="w-full block text-center px-4 py-2 rounded bg-gray-900 text-white hover:bg-black-100 transition mt-2">
                Login
            </a>
        @endif
    </div>
